...
conn = sqlite3.connect(settings.DATABASE_URL)      # conn is now the open connection to my database. we use conn later to refer to that same database connection. 
                                            # conn is an open database file. sqlite3.connect(settings.DATABASE_URL) checks if the file settings.DATABASE_URL exists.
